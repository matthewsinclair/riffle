...
- `Riffle.Service` -- `run/1`: resolve a pipeline source, read rows, run the pattern layer, return a typed result. Closed error vocabulary; the pipeline is resolved _before_ the run so a bad source or name is a tagged error rather than a run that fails halfway.
